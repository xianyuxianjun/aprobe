...
1. **模型不接触被测目标。** 生成阶段的 Agent 只能读规范、写用例文件；通往被测目标的网络出口只存在于确定性的执行阶段，且只接受用例文件里已存在的 `case_id`。（理由：这样「模型不会误伤真实服务」可以一句话证明，而不是靠承诺。）
2. **判定不由模型做出。** 通过/失败只来自声明式 Assertion 的确定性求值；无法求值只能是「无法判定」，不允许退化成「通过」。
3. **事实来源是自有 Trace。** 编排层（LangGraph）的中间状态只用于流转，回放与评估读的是本项目持久化的权威轨迹。（理由：把权威交给框架版本，「重复回放结论一致」就变成框架的行为，而不是你的能力。）
4. **降级模式与 Agent 模式共用同一条流程。** 不调用模型的模式不是「简化版」，而是同一个规划器 seam 的另一个实现：同一套工具、同一份产物结构、同一套判定。两种模式因此可比——质量差值本身就是一个可量化的结果。
...
五个样例集，在仓库自带的两个基准上实跑（`bash scripts/evaluate_all.sh`，回放 2 轮）：
...
| edgecases-defects（按接口锚定，用于规划器对比） | 9 | — | 0 | 1 | — |
...
同一基准（`edgecases-violating`，注入了 8 处可判定的违约）、同一套标注，**只换用例集**。
右侧是**真实模型**（`deepseek-v4-flash`，OpenAI-compatible 端点）产出的用例：
...
| 假阴性（漏报） | 2 | 0 |
| 假阳性（误报） | 0 | 0 |
| 规划成本 | — | 7 步 / 55344 token / 约 29 秒 |

左侧漏掉的两条都是**同一类**：`total` 与 `items` 条数不一致、空列表却声称 `total=0`。
schema 校验对它们无能为力——它能证明"字段是整数"，证明不了"这个整数等于那个数组的长度"。
模型补上的正是这两条，写法是 `$.items` 的 `length_equals_path=$.total`。

**这张表想说的事**：schema 驱动的自动生成覆盖结构与状态码，而**业务规则类违约需要会读语义的用例设计者**。
这就是 Agent 在这条流程里不可替代的位置。

**限度，必须一并读：**

- 单基准、单模型、单次运行。它不构成"某模式更准确"的一般结论。
- 生成本身不可复现（需要同一个模型与端点）；但产出的用例集已固化在 `cases/edgecases-agent.yaml`，
  所以**回放与对比在 CI 里是可复现的**。
- 55344 token 是这一次、这个模型的数字，不代表其他模型。

标注锚在**接口**上而不是用例 id 上（`eval/edgecases-defects.yaml`）。这一点是踩坑之后改的：
模型给用例起的 id 与人工用例不同，按 case_id 标注会让它新加的 7 条用例**全部被忽略**，
对比表显示 +0.0%。

## 接真实模型时发现了什么

第一次把真实模型接进来，抓出四个真实缺陷——**全部在我的代码里，不在模型里**：

1. **工具返回体无上限。** `get_case_history` 对一个 Operation 返回了 20–38 条历史运行，
   而这些内容会在之后每一步里重复发送。一步就把上下文撑到 13738 token。
   现在只回最近若干条 + 汇总。
2. **响应结构打包了整个文档。** `json_schema` 会把 OpenAPI 里**全部** component 塞进 `$defs`，
   而实际用得到的往往只有一两个。改成只打包从该 schema 出发可达的部分。
3. **我把 Agent 的目标写错了。** 提示词要求"为尚未覆盖的 Operation 设计用例"，
   而确定性生成早就把 9 个 Operation 全覆盖了——等于亲手取消了 Agent 的用武之地。
   模型严格照做，一条都没提交，并明确说明理由。现在目标改为**加深验证**：
   工具会报告每条用例的 `assertion_depth`（`schema` / `value` / `rule`），模型据此判断哪里还只停在结构校验上。
4. **断言声明太松。** `assertions` 只约束了 `kind` 是字符串，模型于是写出
   `{"kind": "status", "equals": 200}`（status 只接受 `in: [...]`）和 `"response": 200`（我要求字符串）。
   现在按断言种类用 `oneOf` 精确约束形状，并且**响应码整数与字符串都接受**——
   因为类型不符就拒绝，属于把错误强行制造出来。

另外两处顺带修掉：拒绝原因曾只记成"结构非法"（Trace 里等于没记），
以及循环在预算将尽时什么都不说、任由它撞上限（现在会在 75% 处提醒模型收尾，
把"一无所获"变成"交出部分产出"）。
...
`mock/model_server.py` 只是 chat/completions 的替身，**它不是被测目标**；把两者混为一谈会破坏第 1 条不变量。
