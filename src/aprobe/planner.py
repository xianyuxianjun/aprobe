...
7. 用例**不能设置** `Authorization`、`Cookie`、`Host` 这类请求头：凭据由配置注入，
   用例自己写它们等于伪造身份，会被校验器拒绝。要测"带凭据访问"的行为，
   就让用例只断言状态码——凭据由运行环境提供。
8. 同一个接口可以既写正向用例也写负向用例（缺必填字段应返回 422、凭据有效但资源不存在
   应返回 404 而不是 401）。负向用例往往比正向用例更能发现问题。
9. 覆盖完之后，直接给出停止理由，不要再调用工具。
